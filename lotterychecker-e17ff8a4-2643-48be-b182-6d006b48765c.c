//definition of some error codes and limits
#define ERROR_CODE 1
        ...
        while(fgets(line_of_numbers, 100, fptr)) {
            if (strcmp(line_of_numbers, "\n") == 0) continue;
                ...
                for (int i = 0; i < strlen(line_of_numbers) || strcmp(number, "  ") != 0; i++) {
                    ...
                    else if (strcmp(number, "  ") != 0 && digits > 0) {
                        int num = atoi(number);
                        if (num > 39 || num < 1) {
                            printf("Numbers in the file can only be between 1 and 39 (inclusive), and they must be in the format [num1 num2 num3 num4 num5] for each line (not including empty lines) without the brackets.\n");
                            return ERROR_CODE;
                ...
                *lines = *lines + 1;
                ...
                if (*lines > MAX_NUMBER_LINES) {
                    printf("There should only be 100 lines total with 5 numbers each. This does not include empty lines.\n");
                    return ERROR_CODE;
                }
            }
        }
...
int main(int argc, char* argv[]) {
    ...
    int numbers_to_check[MAX_NUMBER_LINES][NUMBERS_SIZE];
